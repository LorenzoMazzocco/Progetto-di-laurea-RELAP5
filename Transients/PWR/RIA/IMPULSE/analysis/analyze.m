...
for i=1:length(enthalpies)
    filename = sprintf("../enthalpy_%s/out/data.csv", string(enthalpies(i)));
    new = readtable(filename);
    data(i).enthalpy = enthalpies(i);
    data(i).raw_data = new;
end
...
for i=1:length(enthalpies)
    ...
    input_path = sprintf("../enthalpy_%s/input.i", string(enthalpies(i)));
